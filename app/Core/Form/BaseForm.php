...
class BaseForm extends Form
{
    ...
    public function getCodeListWithNameOnly($listName, $listType, $code = true)
    {
        $defaultVersion = config('app.default_version_name');
        $defaultLocale  = config('app.fallback_locale');
        $version        = session()->get('version');
    ...
    protected function addGeneralDescription($className, $label = 'Text', $data = [])
    {
        ...
        return $this->addCollection('narrative', 'Activity\Narrative', $className, $data, trans('elementForm.generalDescription'));
    }
